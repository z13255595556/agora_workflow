...
- **引用**只有企微代运营渠道生效，别的渠道语聚直接忽略这个字段 —— 不报错，没效果。
- **`mention` 认哪个 id 空间文档里没写**，这字段在 `send_text_ex` 里一直只是透传。
  实测**推送的 `user_id`（32 位 hex）不认，回 503**；现在传的是
  `external_contact_id`（企微那套人 id，16 位数字，和「指定发送人」同一个空间）。
  它要是也不被认，`jjy_send` 直接判失败 —— 等于为了一个 @ 把整条回复搞没了。
  所以 `_wf_send` 带 `mention` 发失败时会**摘掉它重发一次**并写一行 warning：

  ```
  带 @ 发送失败(HTTP 503)，摘掉 mention 重发 —— 语聚不认这个 id: [...]（正文里的 @昵称 前缀不受影响）
  ```

  日志里还有这行就说明结构化 @ 这条路走不通，正文前缀那个 `@张兵` 照样在，
  只是没有真 @ 的红点。
