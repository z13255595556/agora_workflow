...
- **`mention` 认哪个 id 空间没验证过**。这字段在 `send_text_ex` 里一直只是透传，
  从没有前端用过。现在传的是推送里的 `user_id`。万一语聚不认会回 `Code=4000`，
  那就等于为了一个 @ 把整条回复搞没了 —— 所以 `_wf_send` 带 `mention` 发失败时会
  **摘掉它重发一次**，并写一行 warning。日志里出现
  `带 @ 发送失败…，摘掉 mention 重发` 就说明 id 空间猜错了，换 `external_contact_id` 再试。
