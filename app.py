...
def jjy_revoke(conversation_id, message_id):
    ...
    ⚠️⚠️ **企微代运营渠道不支持撤回** —— 直接 curl 打接口实测(2026-08-11)：

        {"Code":4000,"Data":"不支持撤回!","Msg":"不支持撤回!"}

    发的是刚回声回来的真 message_id，秒级时限内，照样 4000。官方文档写的
    「支持小红书、企业微信聚合消息撤回」里的"企业微信"指的不是代运营这条链路。
    换 id、换参数都绕不过去，是渠道本身没这个能力 —— 别再去试第二遍了。

    文档没写时限，但各家 IM 基本都有(企微 2 分钟)，超时也会被语聚那边挡掉，
    只是这个渠道还轮不到时限生效。
